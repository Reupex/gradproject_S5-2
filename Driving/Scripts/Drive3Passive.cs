...
public class Drive3Passive : MonoBehaviour
{
    ...
    //싱글톤
    private static Drive3Passive _instance;
    public static Drive3Passive instance
    {
        get { return _instance; }
    }
    private void Awake()
    {
        _instance = GetComponent<Drive3Passive>();
    }
    ...
    */
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "save")
        {
            ...
            //수동
            if (limitSpeed[saveNum - 1] > limitSpeed[saveNum]) //디바이스 왼쪽으로
            {
                temp = (int)(160f / (device.maxValue - device.minValue) * (limitSpeed[saveNum - 1] - limitSpeed[saveNum])*2);
                CarController.instance.m_Topspeed = limitSpeed[saveNum] * 2;
                device.value = limitSpeed[saveNum] * 2 * (device.maxValue - device.minValue) / 160f + device.minValue;
                Serial.instance.Passive();
                Serial.instance.PaMotor("1" + Serial.instance.WriteAngle(temp) + "2000");
            }
            else
            {
                temp = (int)(160f / (device.maxValue - device.minValue) * (limitSpeed[saveNum] - limitSpeed[saveNum-1]) * 2);
                CarController.instance.m_Topspeed = limitSpeed[saveNum] * 2;
                device.value = limitSpeed[saveNum] * 2 * (device.maxValue - device.minValue) / 160f + device.minValue;
                Serial.instance.Passive();
                Serial.instance.PaMotor("0" + Serial.instance.WriteAngle(temp) + "2000");
